...
const App: React.FC = () => {
    ...
    try {
      ...
      if (apiKey) {
        try {
          ...
          console.warn("AI Speech failed.", e);
        }
      }
  ...
  const handleImageGeneration = async () => {
    if (isGeneratingImg || !state.selectedCategory || !apiKey) return;
    const items = state.selectedCategory.items;
        ...
        <div className="flex-1 bg-white flex flex-col animate-in slide-in-from-bottom duration-500 overflow-hidden">
          ...
          <div className="flex-1 p-4 grid grid-cols-3 gap-3 hide-scrollbar overflow-y-auto">
            {ALPHABET.map((letter, idx) => (
              <button key={letter} onClick={() => handleSpeech(letter)} style={{ backgroundColor: RAINBOW_COLORS[idx % RAINBOW_COLORS.length] }} className="aspect-square rounded-2xl shadow-lg flex items-center justify-center active:scale-90 transition-all border-b-[4px] border-black/10">
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (newItems && newItems.length > 0) {
                        const updatedCats = categories.map(c => c.id === state.selectedCategory!.id ? { ...c, items: [...c.items, ...newItems] } : c);
